...
ifeq ($(isNonInteractive), true)
	${COMPOSE_PREFIX_CMD} docker compose ${COMPOSE_ALL_FILES} exec web poetry -C /home/rengine run python3 manage.py createsuperuser --username ${DJANGO_SUPERUSER_USERNAME} --email ${DJANGO_SUPERUSER_EMAIL} --noinput
else
...
changepassword:	## Change password for user
	${COMPOSE_PREFIX_CMD} docker compose ${COMPOSE_ALL_FILES} exec web poetry -C /home/rengine run python3 manage.py changepassword

migrate:		## Apply migrations
	${COMPOSE_PREFIX_CMD} docker compose ${COMPOSE_ALL_FILES} exec web poetry -C /home/rengine run python3 manage.py migrate
